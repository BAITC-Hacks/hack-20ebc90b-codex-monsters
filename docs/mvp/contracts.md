...
- `provenance`: `observed|derived|synthetic|override`; сохраняется на уровне записей/полей. Синтетический набор остаётся `synthetic_demo`. Реальная история с явно введёнными закупщиком условиями остаётся `real_preview`, с `override`, предупреждениями и допущениями в CSV. Нельзя менять идентичность данных, чтобы обойти проверку утверждения.
...
### Дополнение рабочего MVP, 23.09.2026

По запросу пользователя реальный локальный заказ доступен после подготовки
данных закупщиком; прежние общие запреты real-approval заменены этим сценарием.
Это локальное решение и выгрузка, без внешней отправки или проверки ERP.

- `GET /v1/workspace` возвращает сохранённые планы, снимки, источники и последний
  снимок, включая сохранённую подготовку до завершения расчёта; пути к файлам скрыты.
- `SnapshotRequest.sku_limit`: 1–10000, default 20. Зарегистрированные presets
  `systeme-preview-v1` (S07/S08/S12) и `iek-preview-v1` (S01/S02/S05) используют
  только выбранные пути и проверяют их checksum.
- `GET /v1/snapshots/{id}/buyer-inputs` возвращает товары и известные условия;
  неизвестное представлено `null`. `POST` принимает выбранные `items`, `reason`,
  `accept_history_estimate=true`, `idempotency_key` и возвращает новый
  `SnapshotManifest`. SKU/склад/поставщик/базовая единица сверяются с исходником.
- `BuyerInput`: purchase_uom, base_units_per_purchase_uom, quantity_quantum,
  free_base, moq_purchase, pack_multiple_purchase, lead_time_days, review_days;
  optional cost_per_base/currency и incoming_base_qty/incoming_eta.
  `free_base` уже учитывает резерв. Ожидаемая дата означает конец указанного дня.
- В подготовленном снимке master, inventory, terms и pipeline имеют `override`.
  `demand_coverage` явно задаёт допущение нулевого спроса в дни без записей;
  карантин не превращается в наблюдённые движения. Исходный снимок неизменяем.
- `ApprovalRequest.acknowledge_assumptions=true` обязателен для подготовленного
  `real_preview`; подтверждение входит в audit. После правки утверждение снимается.
  CSV содержит только положительные количества, наименование, цены при наличии,
  объяснение, исходные и введённые допущения; маркирован как локальный неотправленный план.
- `POST /v1/planning-runs/{id}/review-events` создаёт отдельный пересчёт с решениями
  по продажам. Неизвестные события и возвраты не принимаются за положительный спрос.
- Минимальный горизонт прогноза 90 дней; API увеличивает его под L+R, max_cover
  и сценарий задержки. Прогноз и planning используют календарные дни UTC.
